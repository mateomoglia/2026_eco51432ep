...
To obtain a panel dataset (multiple IDs observed multiple times), we need to reshape the datasets in **long format** (as opposed to wide). The `dplyr` function is `pivot_longer()`.
...
The counterpart function is `pivot_wider()`.
...
6.  Merge the population and the housing cost datasets on municipality ID
7.  Save the newly merged dataset in `.csv` in an appropriate subfolder
